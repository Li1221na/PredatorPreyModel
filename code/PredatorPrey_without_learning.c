...
// The time in the code and data files is based on update time, using the time interval as a reference. Thus, 1 update time corresponds to 0.1 time on the paper.
...
int r_danger;   // (=r_haz) : Maximal distance for recognizing hazard
...
double **Dis;  // Dis : Distance between chasers and targets
...
int **CatchNumber, **BelongSet, **SetNumber;   // Arrays for data storage; for more details, refer to the "write_file()" function.
...
void calculate_distance(), find_nearest(), check_set(), chaser_velocity(), target_velocity(), chaser_move(), target_move(), record_catch_number();
void write_file_Position(), write_file_CatchSet(), write_file_CatchNumber(), write_file_BelongSet(),write_file_SetNumber();
...
void ini_setting(){
  ...
  for(int i=ini_NCd;i<ini_NC;i++) CS[i] = 1;
...
void write_file_Position(){
  FILE *wf;
  char fn[300];
  sprintf(fn,"../result/01_Position_withoutLearning_M%d_Tmax%d_R%d_NCd%d_NCg%d_NT%d_vdcdt%.3lf_vgcdt%.3lf_vtdt%.3lf_rd%d_ht%d_rs%.2lf_t%d.txt",M,Tmax,R,ini_NCd,ini_NCg,ini_NT,vdcdt,vgcdt,vtdt,r_danger,ht,r_space,t);
  wf = fopen(fn,"w");
...
void write_file_CatchSet(){
  FILE *wf;
  char fn[300];
  sprintf(fn,"../result/02_Catch_withoutLearning_M%d_Tmax%d_R%d_NCd%d_NCg%d_NT%d_vdcdt%.3lf_vgcdt%.3lf_vtdt%.3lf_rd%d_ht%d_rs%.2lf.txt",M,Tmax,R,ini_NCd,ini_NCg,ini_NT,vdcdt,vgcdt,vtdt,r_danger,ht,r_space);
  wf = fopen(fn,"w");
...
void write_file_CatchNumber(){
  FILE *wf;
  char fn[300];
  sprintf(fn,"../result/03_CatchNumber_withoutLearning_M%d_Tmax%d_R%d_NCd%d_NCg%d_NT%d_vdcdt%.3lf_vgcdt%.3lf_vtdt%.3lf_rd%d_ht%d_rs%.2lf.txt",M,Tmax,R,ini_NCd,ini_NCg,ini_NT,vdcdt,vgcdt,vtdt,r_danger,ht,r_space);
  wf = fopen(fn,"w");
...
void write_file_BelongSet(){
  FILE *wf;
  char fn[300];
  sprintf(fn,"../result/04_BelongSet_withoutLearning_M%d_Tmax%d_R%d_NCd%d_NCg%d_NT%d_vdcdt%.3lf_vgcdt%.3lf_vtdt%.3lf_rd%d_ht%d_rs%.2lf.txt",M,Tmax,R,ini_NCd,ini_NCg,ini_NT,vdcdt,vgcdt,vtdt,r_danger,ht,r_space);
  wf = fopen(fn,"w");
...
void write_file_SetNumber(){
  FILE *wf;
  char fn[300];
  sprintf(fn,"../result/05_SetNumber_withoutLearning_M%d_Tmax%d_R%d_NCd%d_NCg%d_NT%d_vdcdt%.3lf_vgcdt%.3lf_vtdt%.3lf_rd%d_ht%d_rs%.2lf.txt",M,Tmax,R,ini_NCd,ini_NCg,ini_NT,vdcdt,vgcdt,vtdt,r_danger,ht,r_space);
  wf = fopen(fn,"w");
